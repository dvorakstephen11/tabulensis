...
- Have multiple models do deep research to estimate revenue for Synkronizer 
...
- Deep dive on PerfectXL, xltrail, xlCompare, Synkronizer, and Ablebits (separately). Each main competitor needs to have its own deep dive document.
...
- Map milestones to product phases
- Put all the competitor deep dives into a single thread and research the following
- What are the tradeoffs in the WASM packaging?
- Revenue projections for my product (add in the YouTube, Reddit, and other user data as well for maximal accuracy)
- Refine or validate the product offering phases
