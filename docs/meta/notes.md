...
- So far I'm pretty sure there's not, but I need to check. xltrail might be the closest, since 
it's web-based and can therefore be accessed on a browser on MacOS.


After each programming cycle, use the [percent_completion.md](percent_completion.md) document to ask an LLM (or two or three) to estimate the percentage of the project that is complete.