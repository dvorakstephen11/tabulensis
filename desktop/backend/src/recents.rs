...
fn save_recents_to_disk(path: &Path, entries: &[RecentComparison]) -> Result<(), DiffErrorPayload> {
    let data = serde_json::to_string_pretty(entries).map_err(|e| {
        DiffErrorPayload::new(
            "recents",
            format!("Failed to serialize recents: {e}"),
            false,
        )
    })?;
    std::fs::write(path, data).map_err(|e| {
        DiffErrorPayload::new("recents", format!("Failed to write recents: {e}"), false)
    })
}

fn update_recents(
    mut entries: Vec<RecentComparison>,
    entry: RecentComparison,
) -> Vec<RecentComparison> {
