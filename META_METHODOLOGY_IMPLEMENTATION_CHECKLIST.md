...
- [ ] Decide whether `docs/meta/logs/**` and `docs/meta/results/**` are committed to git or kept local-only (record the decision in `docs/meta/README.md`).
- [ ] If local-only: add `docs/meta/logs/` (and any other sensitive vendor dump directories) to `.gitignore` and explain the exclusion in `docs/meta/README.md`.
- [ ] If committed: add a short "Privacy + redaction" warning to `docs/meta/README.md` describing what must never be committed.

- [ ] Create file `docs/meta/README.md` describing:
...
- [ ] Create file `docs/meta/logs/daily/_TEMPLATE.md` with sections:
  - "Date"
  - "Top goals (today)"
  ...
  - "Risks noticed"
  - "Next actions"
- [ ] Create file `docs/meta/logs/research/_TEMPLATE.md` with sections:
  - "Query"
  - "Sources"
  - "Findings"
  - "Actionable experiments"
  - "Append-only log entry"
- [ ] Create file `docs/meta/logs/ops/_TEMPLATE.md` with sections:
  - "Runbook executed"
  - "Incident/issue"
  - "Fix"
  - "Follow-ups"
- [ ] Create file `docs/meta/logs/weekly/_TEMPLATE.md` with sections:
  - "Wins"
  - "Misses"
  - "What to stop doing"
...
- [ ] Create file `docs/meta/today.md` (single-day scratchpad, overwritten daily).
- [ ] Decide whether `docs/meta/today.md` is committed to git or ignored (record the decision in `docs/meta/README.md`).
- [ ] If ignored: add `docs/meta/today.md` to `.gitignore` and add `docs/meta/today.example.md` as a committed template.
- [ ] If committed: add a warning section to `docs/meta/today.md` describing what must never be included (secrets, personal data).
- [ ] Add top-of-file warning to `docs/meta/today.md`: "This file is overwritten; do not store durable history here."
- [ ] Add a short section to `docs/meta/today.md` explaining:
  - it is generated by automation (eventually)
  - it is derived from `meta_methodology.md` + open checklists
- [ ] Add a "Timebox" header to `docs/meta/today.md` with explicit fields:
  - available minutes
  - non-negotiable commitments (e.g., Sourcewell)
  - energy level (low/med/high)
- [ ] Add a "Stop when" header to `docs/meta/today.md` (explicit cutoff conditions to prevent overwork).
- [ ] Add link to `docs/meta/today.md` in `meta_methodology.md` under "Daily Schedule".
...
- [ ] Create file `docs/meta/prompts/README.md` that:
...
- [ ] Create prompt `docs/meta/prompts/deep_research_competitor_watch.md` (competitors + substitutes + pricing + recent changes).
- [ ] Create prompt `docs/meta/prompts/deep_research_acquisition_targets.md` (acquirers + outreach paths + rationale).
- [ ] Create prompt `docs/meta/prompts/deep_research_distribution_experiments.md` (channels + 2-hour actions + metrics).
- [ ] Create prompt `docs/meta/prompts/deep_research_security_watch.md` (security news + supply chain + CVEs relevant to stack).
- [ ] Add a 1-paragraph "When to use which prompt" header at top of each deep research prompt file.
- [ ] Ensure every deep research prompt includes a `{{RUN_DATE}}` placeholder (filled by script) and a "citation requirements" section.

### 3.2 Friction Removal For Running Deep Research

- [ ] Add `--prompt <name>` support to `scripts/deep_research_prompt.py` so it can copy different prompt files.
- [ ] Add `--list` support to `scripts/deep_research_prompt.py` to print available prompt names.
- [ ] Add `--open` support to `scripts/deep_research_prompt.py` to open ChatGPT in the default browser (best-effort).
- [ ] Add `--new-result` support to `scripts/deep_research_prompt.py` that creates a new timestamped file in `docs/meta/results/` and prints the path.
- [ ] Add `--copy-result-path` support to copy the intended result file path (so you paste results into the right file).
- [ ] Add `--new-a-b` support to `scripts/deep_research_prompt.py` that creates:
  - `..._<prompt>_a.md`
  - `..._<prompt>_b.md`
  - `..._<prompt>_synthesis.md`
  and prints all three paths.
- [ ] Add `--open-result` support to `scripts/deep_research_prompt.py` that opens the generated result files in the default editor (best-effort).
- [ ] Add `--audio` placeholder support to `scripts/deep_research_prompt.py` that prints a future audio output path (even before TTS exists).

### 3.3 Capturing Results (Two Chat Outputs + Synthesis)

- [ ] Decide whether deep research results live in:
  - `docs/meta/results/` (flat), or
  - `docs/meta/results/deep_research/` (grouped).
  Record the decision in `docs/meta/results/README.md`.
- [ ] Decide a canonical filename scheme for deep research captures (document in `docs/meta/logs/README.md`):
  - date prefix: `YYYY-MM-DD`
  - suffix describes prompt and/or chat: `market_analysis_a`, `market_analysis_b`, `synthesis`
- [ ] Create file `docs/meta/results/README.md` with:
  - naming scheme
  - recommended structure inside each result file (executive brief, findings, sources, actions)
  - "append-only log entry" convention
- [ ] Add an explicit "Where to paste" rule to `docs/meta/results/README.md`:
...
- [ ] Add a "How to capture" section to `meta_methodology.md` describing:
...
- [ ] Create synthesis prompt file `docs/meta/prompts/deep_research_synthesis.md` that takes:
...
- [ ] Add a "post-synthesis action" checklist to `docs/meta/prompts/deep_research_synthesis.md`:
...
- [ ] Decide a TTS path (document in `docs/meta/README.md`):
  - cloud TTS (which provider), or
  - local TTS (which tool).
- [ ] Create directory `docs/meta/audio/` (or pick a different directory; record the decision).
- [ ] Create script `scripts/tts_generate.py` that:
  - takes an input markdown/text file
  - writes an `.mp3` or `.wav` output
  - prints the output path
- [ ] Add `scripts/tts_generate.py` `--voice` option (even if only 1 voice exists initially).
- [ ] Add `scripts/tts_generate.py` `--rate` option (speed control).
- [ ] Add `scripts/tts_generate.py` `--max-chars` option to avoid giant audio files.
- [ ] Add a `docs/meta/audio/README.md` explaining:
...
- [ ] Create script `scripts/new_daily_log.py` that:
  - creates `docs/meta/logs/daily/YYYY-MM-DD.md` from template
  - refuses to overwrite an existing file
  - prints the path to stdout
- [ ] Add `scripts/new_daily_log.py` `--copy-path` option (copies the created path to clipboard for quick open/paste).
- [ ] Add `scripts/new_daily_log.py` `--open` option (opens the created file in the default editor, best-effort).
- [ ] Create script `scripts/new_ops_log.py` that:
...
- [ ] Create script `scripts/new_research_log.py` that:
...
- [ ] Add to `meta_methodology.md` a single "Daily close-out" rule:
...
- [ ] Add to `meta_methodology.md` a single "Daily open" rule:
...
- [ ] Decide canonical inputs to daily planning (document in `docs/meta/logs/README.md`):
  - `meta_methodology.md`
...
- [ ] Create file `docs/meta/prompts/daily_plan.md` that:
...
- [ ] Create script `scripts/generate_daily_plan_context.py` that prints:
...
- [ ] Ensure `scripts/generate_daily_plan_context.py` has a `--max-chars` option to avoid runaway output.
- [ ] Ensure `scripts/generate_daily_plan_context.py` has a `--copy` option to copy output to clipboard (fallback to `tmp/` backup).
- [ ] Ensure `scripts/generate_daily_plan_context.py` includes (at minimum) these file contents when present:
  - `docs/index.md`
  - `AGENTS.md`
  - `meta_methodology.md`
...
- [ ] Add a short usage block in `meta_methodology.md`:
...
- [ ] Create file `docs/meta/prompts/pro_context_payload.md` that defines:
...
- [ ] Decide where context payload outputs live:
...
- [ ] Create directory `docs/meta/automation/`.
- [ ] Create file `docs/meta/automation/README.md` defining:
  - what counts as a "safe" automated action
  - what must be human-reviewed
  - how to record failures
- [ ] Create file `docs/meta/automation/guardrails.md` defining:
...
- [ ] Decide a single output directory for automated runs (document in `docs/meta/automation/README.md`).
- [ ] Decide a filename scheme: `YYYY-MM-DD_HHMM_<task>_<agent>.md`.
...
- [ ] Decide whether automation runs in:
  - current worktree (fast, risky), or
  - dedicated git worktrees (safer)
- [ ] If worktrees: create script `scripts/agent_worktree.sh` that:
  - creates `../excel_diff_worktrees/<name>` (outside repo) with `git worktree add`
  - prints the path
  - has a `--cleanup` command to remove old worktrees
- [ ] Add documentation in `docs/meta/automation/README.md` for using `scripts/agent_worktree.sh`.
...
- [ ] Decide the scheduler for the midnight run (record in `docs/meta/automation/README.md`):
  - cron, or
  - systemd timer, or
...
- [ ] If Task Scheduler: create `docs/meta/automation/task_scheduler_example.md` with:
  - trigger settings
  - action command line
...
- [ ] Create directory `docs/meta/marketing_templates/`.
- [ ] Create file `docs/meta/marketing_templates/README.md` describing:
  - what each template is for
  - where generated assets live
  - how to measure results
- [ ] Create file `docs/meta/marketing_templates/post_short_demo.md` with:
  - hook line template
  - 3 bullet body template
  - CTA template (download link + docs link)
  - hashtag/tag guidance
- [ ] Create file `docs/meta/marketing_templates/post_before_after.md` with:
  - "before" screenshot caption template
  - "after" screenshot caption template
  - CTA template
- [ ] Create file `docs/meta/marketing_templates/post_user_story.md` with:
  - target persona
  - pain point
...
- [ ] Decide a canonical UTM scheme (document in `docs/meta/marketing_templates/README.md`):
  - `utm_source`, `utm_medium`, `utm_campaign`, `utm_content`
- [ ] Create script `scripts/utm.py` that:
  - takes source/medium/campaign/content
  - outputs a full URL with UTMs
  - optionally copies to clipboard
- [ ] Create file `docs/meta/logs/marketing/metrics.csv` (or `.md`) with columns:
  - date
  - channel
  - post_id/link
  - impressions
...
- [ ] Decide the minimum demo-video format:
  - 30-60 seconds, or
  - 2-3 minutes.
  Record decision in `docs/meta/marketing_templates/README.md`.
...
- [ ] Decide access method for Fastmail:
  - IMAP, or
  - Fastmail API (if available), or
  - manual export
...
- [ ] Create file `docs/meta/automation/model_accounts.md` listing:
  - which model accounts exist
  - what each is used for
  - rate/usage limits observed
  - when to switch accounts
- [ ] Decide whether to create a second OpenAI Pro account (record decision + date + rationale in `docs/meta/automation/model_accounts.md`).
...
- [ ] Create file `SECURITY_DAILY_CHECKLIST.md`.
- [ ] Add it to `docs/index.md` under Operating docs.
- [ ] Add explicit tasks:
  - dependency audit (Rust + npm)
  - secret scanning (pre-commit/CI)
  - minimal SAST (what tool, how often)
- [ ] Create `scripts/security_audit.sh` that runs the chosen tools and writes a report file under `docs/meta/logs/ops/`.

- [ ] In `SECURITY_DAILY_CHECKLIST.md`, list the exact commands to run for:
  - Rust dependency audit (choose tool)
  - npm dependency audit (choose tool)
  - secret scanning (choose tool)
  - SAST (choose tool)
- [ ] Decide and record the chosen security tools in `docs/meta/automation/guardrails.md`.
- [ ] Add a weekly task in `docs/meta/automation/tasks.yaml` to run `scripts/security_audit.sh`.
- [ ] Add a "redaction rules" section to `SECURITY_DAILY_CHECKLIST.md`:
...
- [ ] Create a short "Methodology DoD" section at the bottom of `meta_methodology.md` that is measurable (examples):
...
- [ ] Decide the "stale checklist" threshold (N days) and record it in `meta_methodology.md` DoD.
