#!/usr/bin/env python3
"""
Create an ops log from a template (file-based when present, otherwise built-in).

Creates:
  docs/meta/logs/ops/YYYY-MM-DD_<slug>.md

Rules:
  - Requires an explicit <slug> argument (example: stripe_webhook_debug).
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Create an ops log from template.")
    parser.add_argument("slug", type=_parse_slug, help="Identifier in lower_snake_case (e.g., stripe_webhook_debug).")
    args = parser.parse_args(argv)

    repo_root = Path(__file__).resolve().parent.parent
    template_path = repo_root / "docs/meta/logs/ops/_TEMPLATE.md"
    template_text = DEFAULT_TEMPLATE
    if template_path.exists():
        template_text = template_path.read_text(encoding="utf-8", errors="replace")
        template_text = template_text.replace("\r\n", "\n").replace("\r", "\n")

    out_dir = repo_root / "docs/meta/logs/ops"
